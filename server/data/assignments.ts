...
export async function listStudentAssignments(
  ...
  if (status && !valid.has(status))
    throw new ApiError("VALIDATION_ERROR", 422, "Status tugas tidak valid.", {
      status: "Gunakan not_started, draft, submitted, atau graded.",
    });
...
export async function getStudentAssignment(
  ...
  const row = await env.DB.prepare(`${assignmentSql} AND a.id = ? LIMIT 1`)
...
export async function saveAssignmentDraft(
  ...
  if (assignment.submission_status === "graded")
    throw new ApiError(
      "CONFLICT",
      409,
      "Tugas yang sudah dinilai tidak dapat diubah.",
    );
  if (assignment.submission_status === "submitted")
    throw new ApiError(
      "CONFLICT",
      409,
      "Tugas yang sudah dikirim tidak dapat diubah.",
    );
  ...
  await env.DB.prepare(
    ...
    .bind(crypto.randomUUID(), assignmentId, user.id, answerText, now, now)
    .run();
...
export async function submitAssignment(
  ...
  if (
    ...
    return assignment;
  if (!assignment.answer_text?.trim())
    throw new ApiError(
      "VALIDATION_ERROR",
      422,
      "Jawaban wajib diisi sebelum tugas dikirim.",
      { answerText: "Simpan jawaban terlebih dahulu." },
    );
  const now = new Date();
  if (!assignment.allow_late && now > new Date(assignment.due_at))
    throw new ApiError(
      "CONFLICT",
      409,
  ...
  await env.DB.prepare(
    ...
    WHERE assignment_id = ? AND student_id = ? AND status = 'draft'`,
  )
    .bind(isoNow, isoNow, assignmentId, user.id)
    .run();
  return getStudentAssignment(user, assignmentId);
}
