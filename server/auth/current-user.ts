...
function developmentUser(env: AuthEnvironment): CurrentUser | null {
  if (env.RUNTIME_ENV !== "development" || env.RT_DEV_AUTH !== "enabled")
    return null;
  ...
  if (!role || !roles.has(role as UserRole)) {
    throw new AuthError(
      "UNAUTHENTICATED",
      401,
      "Development identity has an invalid role.",
    );
  }
  return {
...
export async function getCurrentUser(
  ...
  env: AuthEnvironment,
  lookup: UserLookup,
  lookupByEmail?: UserLookupByEmail,
  ...
  if (email) {
    const byExternalIdentity = externalIdentityId
      ? await lookup(externalIdentityId)
      : null;
    if (byExternalIdentity && byExternalIdentity.email.toLowerCase() !== email)
      return null;
...
export async function requireCurrentUser(
  ...
  if (!user)
    throw new AuthError("UNAUTHENTICATED", 401, "Sign in is required.");
  return user;
}
