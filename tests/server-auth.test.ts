...
const storedStudent: StoredUser = {
  id: "student-1",
...
test("platform identity rejects unknown email and mismatched id-email pairs", async () => {
  const unknown = new Request("https://local.test", {
    headers: { "oai-authenticated-user-email": "unknown@example.test" },
  });
  assert.equal(
  ...
  const mismatch = new Request("https://local.test", {
    headers: {
      "oai-authenticated-user-id": "ext-1",
