...
export async function GET(request: Request) {
  try {
    ...
    if (user.role === 'student') return apiSuccess(await getStudentProfile(user));
    requireRole(user, 'teacher', 'admin');
    return apiSuccess({ id:user.id, email:user.email, displayName:user.displayName, role:user.role, enrollment:null });
  }
