...
const shortDateTime = new Intl.DateTimeFormat("id-ID", {
  dateStyle: "medium",
  timeStyle: "short",
...
export default function Home() {
  ...
  const [submitted, setSubmitted] = useState<number[]>(() => {
    try {
      return typeof window === "undefined"
        ? []
        : JSON.parse(localStorage.getItem("rt-submitted") || "[]");
    } catch {
      return [];
    }
  });
  const [savedBooks, setSavedBooks] = useState<number[]>(() => {
    try {
      return typeof window === "undefined"
        ? []
        : JSON.parse(localStorage.getItem("rt-books") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const [quizServerResult, setQuizServerResult] =
    useState<QuizServerResult | null>(null);
  const [quizSaving, setQuizSaving] = useState(false);
  const [dailyGoal, setDailyGoal] = useState(() => {
    try {
      return typeof window === "undefined"
        ? 30
        : Number(localStorage.getItem("rt-daily-goal") || 30);
    } catch {
      return 30;
    }
  });
  const [reminders, setReminders] = useState(() => {
    try {
      return typeof window === "undefined"
        ? true
        : localStorage.getItem("rt-reminders") !== "false";
    } catch {
      return true;
    }
  });
  const [readingMode, setReadingMode] = useState(() => {
    try {
      return typeof window === "undefined"
        ? "Nyaman"
        : localStorage.getItem("rt-reading-mode") || "Nyaman";
    } catch {
      return "Nyaman";
    }
  });
  ...
  useEffect(() => {
    ...
    read<ProfileData>("/api/v1/me")
      .then(async (profile) => {
        setProfileData(profile);
        if (profile.role === "teacher") {
          const submissions = await read<TeacherSubmission[]>(
        ...
        const [dashboard, materials, books, studentAssignments, quizPayload] =
          await Promise.all([
            read<DashboardData>("/api/v1/dashboard"),
            read<MaterialData[]>("/api/v1/materials"),
            read<LibraryData[]>("/api/v1/library"),
            read<AssignmentData[]>("/api/v1/assignments"),
            read<QuizData>("/api/v1/quizzes/quiz_surabaya"),
          ]);
        setDashboardData(dashboard);
        setMaterialData(materials);
        setLibraryData(books);
        setQuizData(quizPayload);
        setAssignmentData(studentAssignments);
        setAssignmentAnswers(
          Object.fromEntries(
            studentAssignments.map((task) => [task.id, task.answer_text ?? ""]),
          ),
        );
        setSavedBooks(
          books
            .filter((book) => Boolean(book.bookmarked))
            .map((_, index) => index + 1),
        );
        setApiStatus("ready");
      })
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/quizzes/${quizData.id}/attempts`, { method: "POST" })
      .then(async (response) => {
        const payload = (await response.json()) as ApiEnvelope<QuizAttempt>;
        if (!response.ok || payload.error)
          throw new Error(payload.error?.message ?? "Latihan gagal dimulai.");
        return payload.data as QuizAttempt;
      })
      .then((attempt) => setQuizAttemptId(attempt.id))
      .catch((error) =>
        setNotice(
  ...
  const toggleBook = async (id: number | string) => {
    if (typeof id === "number") {
      const next = savedBooks.includes(id)
        ? savedBooks.filter((x) => x !== id)
        : [...savedBooks, id];
      setSavedBooks(next);
    ...
    try {
      const response = await fetch(`/api/v1/library/${id}/progress`, {
        method: "PUT",
      ...
      if (!response.ok || payload.error)
        throw new Error(payload.error?.message ?? "Buku gagal disimpan.");
  ...
  const advanceLibrary = async (id: string) => {
    const book = libraryData.find((item) => item.id === id);
    if (!book) return;
    ...
    try {
      ...
      const payload = (await response.json()) as ApiEnvelope<unknown>;
      if (!response.ok || payload.error)
        throw new Error(payload.error?.message ?? "Progres gagal disimpan.");
      setLibraryData((current) =>
        current.map((item) =>
          item.id === id
      ...
      setNotice(`Progres buku diperbarui menjadi ${percent}%.`);
    } catch (error) {
      setNotice(
  ...
  const saveMaterialProgress = async () => {
    const material = materialData[0];
    if (!material) {
      setBookmarked(!bookmarked);
      return;
    }
    setSavingProgress(material.id);
    try {
      const response = await fetch(
        `/api/v1/materials/${material.id}/progress`,
        {
          method: "PUT",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
      ...
      if (!response.ok || payload.error)
        throw new Error(
          payload.error?.message ?? "Progres materi gagal disimpan.",
        );
      setBookmarked(true);
      setNotice("Posisi dan progres materi disimpan ke akun belajar.");
    } catch (error) {
      setNotice(
  ...
  const submitTask = async (id: number | string) => {
    if (typeof id === "number") {
      const next = [...new Set([...submitted, id])];
      setSubmitted(next);
      localStorage.setItem("rt-submitted", JSON.stringify(next));
      setNotice("Tugas tersimpan sebagai terkirim di perangkat ini.");
      return;
    }
    ...
    try {
      const write = async <T,>(url: string, method: string, body?: unknown) => {
        ...
        if (!response.ok || payload.error)
          throw new Error(payload.error?.message ?? "Tugas gagal dikirim.");
        return payload.data as T;
      };
      await write<AssignmentData>(
        `/api/v1/assignments/${id}/submission`,
        "PUT",
        { answerText: assignmentAnswers[id] ?? "" },
      );
      const updated = await write<AssignmentData>(
      ...
      setNotice("Tugas berhasil dikirim ke server lokal.");
    } catch (error) {
      setNotice(
  ...
  const selectQuizOption = async (questionId: string, optionId: string) => {
    ...
    setQuizSelections((current) => ({ ...current, [questionId]: optionId }));
    setQuizSaving(true);
    try {
      const response = await fetch(
        `/api/v1/quiz-attempts/${quizAttemptId}/answers/${questionId}`,
        {
          method: "PUT",
          headers: { "content-type": "application/json" },
      ...
      const payload = (await response.json()) as ApiEnvelope<unknown>;
      if (!response.ok || payload.error)
        throw new Error(payload.error?.message ?? "Jawaban gagal disimpan.");
    } catch (error) {
      setNotice(
  ...
  const submitServerQuiz = async () => {
    ...
    try {
      const submittedResponse = await fetch(
        `/api/v1/quiz-attempts/${quizAttemptId}/submit`,
        { method: "POST" },
      );
      const submitted =
        (await submittedResponse.json()) as ApiEnvelope<QuizAttempt>;
      if (!submittedResponse.ok || submitted.error)
        throw new Error(submitted.error?.message ?? "Latihan gagal dikirim.");
      const resultResponse = await fetch(
        `/api/v1/quiz-attempts/${quizAttemptId}/result`,
      );
      const result =
        (await resultResponse.json()) as ApiEnvelope<QuizServerResult>;
      if (!resultResponse.ok || result.error)
        throw new Error(result.error?.message ?? "Hasil latihan gagal dimuat.");
      setQuizServerResult(result.data as QuizServerResult);
    } catch (error) {
      setNotice(
  ...
  const restartServerQuiz = () => {
    setQuizStep(0);
    setQuizSelections({});
    setQuizServerResult(null);
    setQuizAttemptId(null);
  };
  const saveGrade = async (submissionId: string) => {
    const grade = teacherGrades[submissionId];
    if (!grade) return;
    ...
    try {
      ...
      const payload = (await response.json()) as ApiEnvelope<TeacherSubmission>;
      if (!response.ok || payload.error)
        throw new Error(payload.error?.message ?? "Penilaian gagal disimpan.");
      const updated = payload.data as TeacherSubmission;
      setTeacherSubmissions((current) =>
        current.map((item) => (item.id === submissionId ? updated : item)),
      );
      setNotice("Nilai dan umpan balik berhasil disimpan.");
    } catch (error) {
      setNotice(
  ...
  const shownLibrary = (
    libraryData.length
      ...
      : library.map((book) => ({
          ...book,
          bookmarked: savedBooks.includes(book.id),
        }))
  ).filter((book) =>
    `${book.title} ${book.author}`.toLowerCase().includes(query.toLowerCase()),
  );
  const navigation =
    profileData?.role === "teacher" ? ["Penilaian", "Profil"] : studentNav;
            ...
            <span className="profile-copy">
              ...
              <small>
                {profileData?.enrollment
                  ? `${profileData.enrollment.program} · Kelas ${profileData.enrollment.grade_level}`
                  : "Profil siswa"}
              </small>
            </span>
          </button>
          ...
          <>
            <section className="welcome">
              <div>
                ...
                <p>
                  Mulai dari yang kecil. Satu materi hari ini adalah satu
                  langkah maju.
                </p>
              </div>
            ...
            <section className="continue-card" aria-labelledby="continue-title">
              ...
              <div className="continue-copy">
                ...
                <p>
                  {continueMaterial
                    ? `${continueMaterial.subject}${continueMaterial.last_position ? ` · ${continueMaterial.last_position}` : ""}`
                    : "Sejarah Indonesia · Bab 3"}
                </p>
                <div className="progress-row">
                  <div className="progress">
                    <span
                      style={{ width: `${continueMaterial?.percent ?? 68}%` }}
                    />
                  </div>
                  <b>{continueMaterial?.percent ?? 68}%</b>
                </div>
              </div>
          ...
          <section className="reader-page">
            <header className="inner-header">
              <div>
                <p className="eyebrow">
                  {materialData[0]
                    ? `${materialData[0].subject.toUpperCase()} · MATERI ${materialData[0].order_index}`
                    : "SEJARAH INDONESIA · BAB 3"}
                </p>
                <h1>
                  {materialData[0]?.title ?? "Mempertahankan Kemerdekaan"}
                </h1>
                <p>
                  {materialData[0]?.last_position
                    ? `Terakhir dibaca: ${materialData[0].last_position}`
                    : (materialData[0]?.summary ??
                      "Terakhir dibaca: halaman 12 dari 28")}
                </p>
              </div>
              <button
                className={bookmarked ? "bookmark saved" : "bookmark"}
                disabled={savingProgress === materialData[0]?.id}
                onClick={saveMaterialProgress}
                aria-pressed={bookmarked}
              >
                <span aria-hidden="true">{bookmarked ? "★" : "☆"}</span>
                {savingProgress === materialData[0]?.id
                  ? "Menyimpan…"
                  : bookmarked
                    ? "Progres tersimpan"
                    : "Simpan progres"}
            ...
            <div className="reader-layout">
              ...
              <article className="reader" style={{ fontSize }}>
                ...
                <p>
                  Pertempuran Surabaya merupakan salah satu peristiwa penting
                  dalam sejarah perjuangan mempertahankan kemerdekaan Indonesia.
                  Perlawanan rakyat menunjukkan bahwa kemerdekaan bukan sekadar
                  pernyataan, tetapi sesuatu yang akan dipertahankan bersama.
                </p>
                <aside className="key-note">
                  <b>Gagasan utama</b>
                  <p>
                    Perjuangan terjadi melalui perlawanan fisik sekaligus
                    diplomasi untuk memperoleh pengakuan dunia.
                  </p>
                </aside>
                <h3>Mengapa peristiwa ini penting?</h3>
                <p>
                  Semangat perlawanan menarik perhatian internasional dan
                  memperkuat legitimasi Republik Indonesia. Tanggal 10 November
          ...
          (quizData ? (
            <section className="quiz-page">
              <header className="inner-header">
                <div>
                  <p className="eyebrow">
                    LATIHAN PEMAHAMAN · {quizData.subject.toUpperCase()}
                  </p>
                  <h1>{quizData.title}</h1>
                  <p>
                    {quizServerResult
                      ? "Hasil latihan tersimpan di server."
                      : `Soal ${quizStep + 1} dari ${quizData.questions.length} · Jawaban disimpan otomatis.`}
                  </p>
                </div>
                <span className="quiz-count">
                  {quizServerResult
                    ? quizServerResult.attempt.score
              ...
              <div className="quiz-card">
                {quizServerResult ? (
                  <>
                    <h2>Nilai kamu: {quizServerResult.attempt.score}</h2>
                    <p>
                      {quizServerResult.passed
                        ? "Kamu mencapai nilai kelulusan."
                        : `Nilai kelulusan adalah ${quizServerResult.passingScore}. Coba pelajari kembali materinya.`}
                    </p>
                    <div className="quiz-review">
                      {quizServerResult.answers.map((answer) => (
                        <div
                          className={
                            answer.is_correct
                              ? "feedback correct"
                    ...
                    <div className="quiz-actions">
                      ...
                      <button className="primary" onClick={restartServerQuiz}>
                        Ulangi latihan
                      </button>
                    </div>
                  </>
                ) : quizAttemptId && quizData.questions[quizStep] ? (
                  <>
                    <div className="quiz-progress">
                      <span
                        style={{
                          width: `${((quizStep + 1) / quizData.questions.length) * 100}%`,
                        }}
                      />
                    </div>
                    <fieldset>
                      <legend>{quizData.questions[quizStep].prompt}</legend>
                      {quizData.questions[quizStep].options.map(
                        (option, index) => (
                          <label
                            className={`option ${quizSelections[quizData.questions[quizStep].id] === option.id ? "selected" : ""}`}
                            key={option.id}
                          >
                            <input
                              type="radio"
                              ...
                              checked={
                                quizSelections[
                                  quizData.questions[quizStep].id
                                ] === option.id
                              }
                              onChange={() =>
                                selectQuizOption(
                    ...
                    <div className="quiz-actions">
                      <button
                        className="secondary"
                        onClick={() =>
                          quizStep ? setQuizStep(quizStep - 1) : goTo("Materi")
                        }
                      >
                        {quizStep ? "← Soal sebelumnya" : "← Buka materi"}
                      </button>
                      <button
                        className="primary"
                        disabled={
                          quizSaving ||
                          !quizSelections[quizData.questions[quizStep].id]
                        }
                        onClick={() =>
                          quizStep < quizData.questions.length - 1
              ...
              <header className="inner-header">
                <div>
                  ...
                  <p>
                    {quizResult
                      ? "Hasil latihan"
                      : `Soal ${quizStep + 1} dari 5 · Pilih satu jawaban.`}
                  </p>
                </div>
                <span className="quiz-count">
                  {quizResult
                    ? `${quizAnswers.filter((a, i) => a === quiz[i][2]).length * 20}`
                    : `${quizStep + 1} / 5`}
                </span>
              </header>
              <div className="quiz-card">
                {quizResult ? (
                  <>
                    <h2>
                      Nilai kamu:{" "}
                      {quizAnswers.filter((a, i) => a === quiz[i][2]).length *
                        20}
                    </h2>
                    <p>
                      Backend latihan tidak tersedia; hasil hanya tersimpan di
                      sesi ini.
                    </p>
                    <div className="quiz-actions">
                      <button
                  ...
                  <>
                    <div className="quiz-progress">
                      <span style={{ width: `${(quizStep + 1) * 20}%` }} />
                    </div>
                    <fieldset>
                      <legend>{quiz[quizStep][0]}</legend>
                      {quiz[quizStep][1].map((label, index) => (
                        <label
                          className={`option ${quizAnswers[quizStep] === index ? "selected" : ""}`}
                          key={label}
                        >
                          <input
                            type="radio"
                            name="answer"
                            checked={quizAnswers[quizStep] === index}
                            onChange={() =>
                              setQuizAnswers([
                                ...quizAnswers.slice(0, quizStep),
                                index,
                              ])
                            }
                          />
                    ...
                    <div className="quiz-actions">
                      ...
                      >
                        {quizStep < 4 ? "Soal berikutnya →" : "Lihat hasil"}
                      </button>
                    </div>
                  </>
                )}
        ...
        {active === "Tugas" && (
          <section>
            ...
            <div className="assignment-grid">
              {(assignmentData.length ? assignmentData : assignments).map(
                (task) => {
                    ...
                    <article className="assignment-card" key={task.id}>
                      ...
                      {status === "graded" ? (
                        ...
                        <>
                          ...
                          <button
                            className="primary"
                            disabled={savingAssignment === String(task.id)}
                            onClick={() => submitTask(task.id)}
                          >
                            {savingAssignment === String(task.id)
                              ? "Mengirim…"
                              : "Kirim tugas"}
                          </button>
                        </>
          ...
          <section>
            <header className="inner-header">
              <div>
                ...
                <p>
                  Periksa jawaban, berikan nilai, dan kirim umpan balik yang
                  jelas.
                </p>
              </div>
            ...
            {teacherSubmissions.length ? (
              <div className="grading-list">
                {teacherSubmissions.map((item) => {
                  const grade = teacherGrades[item.id] ?? {
                    score: "",
                    feedback: "",
                  };
                  return (
                  ...
                  <div>
                    ...
                    <div className="book-actions">
                      <button
                        ...
                        disabled={savingProgress === String(book.id)}
                        onClick={() =>
                          typeof book.id === "string"
                            ? advanceLibrary(book.id)
                            : goTo("Materi")
                        }
                      >
                        {typeof book.id === "string"
          ...
          <section>
            <header className="inner-header">
              <div>
                ...
                <p>
                  Identitas dan progres berasal dari akun belajar. Preferensi
                  tetap disimpan di perangkat ini.
                </p>
              </div>
            </header>
            <div className="profile-settings-grid">
              <article className="student-profile-card">
                ...
                <div>
                  ...
                  <small>{profileData?.email ?? "Memuat identitas…"}</small>
                </div>
                <dl>
                  <div>
                    <dt>Rangkaian</dt>
                  ...
                  <div>
                    ...
                    <dd>{dashboardData?.progress?.started ?? 0}</dd>
                  </div>
                  <div>
                    <dt>Nilai rata-rata</dt>
                    <dd>
                      {Math.round(
                        dashboardData?.progress?.average_percent ?? 0,
                      )}
                    </dd>
                  </div>
                </dl>
              ...
              <article className="preferences-card">
                <div>
                  ...
                  <p>
                    Pilih durasi yang realistis agar belajar tetap konsisten.
                  </p>
                    ...
                    {[15, 30, 45, 60].map((goal) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setNotice(
                            `Target belajar diubah menjadi ${goal} menit.`,
                          );
                        }}
                      >
                        {goal} menit
                ...
                <button
                  ...
                  onClick={() => {
                    localStorage.removeItem("rt-submitted");
                    localStorage.removeItem("rt-books");
                    localStorage.removeItem("rt-daily-goal");
                    localStorage.removeItem("rt-reminders");
                    localStorage.removeItem("rt-reading-mode");
                    setSubmitted([]);
                    setSavedBooks([]);
                    setDailyGoal(30);
                    setReminders(true);
                    ...
                    setNotice(
                      "Progres dan preferensi lokal telah diatur ulang.",
                    );
                  }}
                >
