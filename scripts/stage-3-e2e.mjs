...
function run(command, args) {
  execFileSync(command, args, { cwd:new URL('..', import.meta.url), stdio:'inherit' });
}

function resetFixture() {
  run('npm', ['run', 'db:local:seed']);
  const wrangler = ['wrangler', 'd1', 'execute', 'site-creator-d1', '--local', '--config', 'wrangler.local.jsonc', '--command'];
  run('npx', [...wrangler, "DELETE FROM submissions WHERE assignment_id = 'asg_refleksi' AND student_id = 'usr_student_dudin'"]);
  run('npx', [...wrangler, "DELETE FROM quiz_answers WHERE attempt_id IN (SELECT id FROM quiz_attempts WHERE quiz_id = 'quiz_surabaya' AND student_id = 'usr_student_dudin')"]);
  run('npx', [...wrangler, "DELETE FROM quiz_attempts WHERE quiz_id = 'quiz_surabaya' AND student_id = 'usr_student_dudin'"]);
}

async function request(path, init, expectedStatus = 200) {
  const response = await fetch(`${baseUrl}${path}`, init);
  const payload = await response.json();
  assert.equal(response.status, expectedStatus, `${init?.method ?? 'GET'} ${path}: ${JSON.stringify(payload)}`);
  return payload;
}
...
async function stopServer() {
  ...
  if (server.exitCode === null && server.signalCode === null) killGroup('SIGTERM');
  server = undefined;
}
...
try {
  ...
  await request('/api/v1/materials/mat_surabaya/progress', {
    method:'PUT', headers:{ 'content-type':'application/json' }, body:JSON.stringify({ percent:72, lastPosition:'e2e-halaman-13' }),
  });
  await request('/api/v1/library/lib_kemerdekaan/progress', {
    method:'PUT', headers:{ 'content-type':'application/json' }, body:JSON.stringify({ percent:55, lastPosition:'e2e-halaman-70', bookmarked:true }),
  });
  await request('/api/v1/assignments/asg_refleksi/submission', {
    method:'PUT', headers:{ 'content-type':'application/json' }, body:JSON.stringify({ answerText:'Semangat persatuan diwujudkan melalui belajar, gotong royong, dan tanggung jawab.' }),
  });
  const submitted = (await request('/api/v1/assignments/asg_refleksi/submission/submit', { method:'POST' })).data;
  ...
  const attempt = (await request('/api/v1/quizzes/quiz_surabaya/attempts', { method:'POST' })).data;
  await request(`/api/v1/quiz-attempts/${attempt.id}/answers/qq_surabaya_1`, {
    method:'PUT', headers:{ 'content-type':'application/json' }, body:JSON.stringify({ selectedOptionId:'qo_sby_1b' }),
  });
  await request(`/api/v1/quiz-attempts/${attempt.id}/answers/qq_surabaya_2`, {
    method:'PUT', headers:{ 'content-type':'application/json' }, body:JSON.stringify({ selectedOptionId:'qo_sby_2a' }),
  });
  const quizAttempt = (await request(`/api/v1/quiz-attempts/${attempt.id}/submit`, { method:'POST' })).data;
  assert.equal(quizAttempt.score, 100);
  const quizResult = (await request(`/api/v1/quiz-attempts/${attempt.id}/result`)).data;
  ...
  const queue = (await request('/api/v1/teacher/submissions?status=submitted')).data;
  assert.equal(queue.length, 1);
  assert.equal(queue[0].id, submitted.submission_id);
  const graded = (await request(`/api/v1/submissions/${queue[0].id}/grade`, {
    method:'PUT', headers:{ 'content-type':'application/json' }, body:JSON.stringify({ score:91, feedback:'Jawaban jelas dan relevan. Tambahkan contoh dari lingkungan sekitar.' }),
  })).data;
  ...
  console.log(JSON.stringify({
    student:student.email, teacher:teacher.email, materialProgress:72, libraryProgress:55,
    assignmentStatus:result.submission_status, assignmentScore:result.score, quizScore:quizAttempt.score,
